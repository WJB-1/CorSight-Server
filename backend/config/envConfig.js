...
const osmConfig = {
  /** osmium 可执行文件路径 */
  OSMIUM_PATH: getEnv('OSMIUM_PATH', path.join(projectRoot, 'tools', 'osmium.exe')),
  /** osm_data 工作目录 */
  OSM_DATA_DIR: getEnv('OSM_DATA_DIR', path.join(projectRoot, 'osm_data')),
  /** 省级底图 PBF（只读，用于裁剪） */
  ...
  /** GraphHopper JAR 路径 */
  GRAPHHOPPER_JAR: getEnv('GRAPHHOPPER_JAR', path.join(projectRoot, 'graphhopper', 'graphhopper-web-10.0.jar')),
  /** GraphHopper 配置文件 */
  GRAPHHOPPER_CONFIG: getEnv('GRAPHHOPPER_CONFIG', path.join(projectRoot, 'graphhopper', 'config.yml')),
  /** 累积多少个 pending 标签后自动触发注入（0=禁用） */
