...
async function getPending(req, res) {
  try {
    const batches = await batchQueryService.findPending();
...
async function getStats(req, res) {
  try {
    const stats = await batchQueryService.countByStatus();
...
async function getDetail(req, res) {
  try {
    const batch = await batchQueryService.findByBatchId(req.params.batchId);
    if (!batch) {
