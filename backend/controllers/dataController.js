...
async function getPendingTags(req, res) {
  try {
    const tags = await semanticTagService.findPending();
...
async function getTagStats(req, res) {
  try {
    const stats = await semanticTagService.countByStatus();
...
async function triggerInjection(req, res) {
  try {
    ...
    setImmediate(() => {
      injectionService.runInjectionPipeline(options).catch((err) => {
