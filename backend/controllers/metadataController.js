...
/**
 * 创建上传 session
 * @param {object} req.body - { point_id, location:{lat,lng}, crs?, scene_description?, images:[{bearing,fov?,description?}] }
 */
async function create(req, res) {
  try {
    const { point_id, location, crs, images } = req.body;
    ...
    // crs 可选，默认 GCJ02；如提供则校验
    const validCrs = ['WGS84', 'GCJ02', 'BD09'];
    if (crs && !validCrs.includes(crs)) {
      return res.status(400).json({ success: false, error: 'invalid_crs', message: `crs 必须为 ${validCrs.join('/')}` });
    }
